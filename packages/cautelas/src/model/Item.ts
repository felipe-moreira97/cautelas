...
export default class Item extends Entidade<Item, ItemProps> {
    ...
    constructor(props: ItemProps) {
        super(props)
        this.numeroDeSerie = props.numeroDeSerie ? new TextoSimles(props?.numeroDeSerie, 3, 120, 'nome', 'Item') : undefined
        this.categoria = new Categoria(props.categoria)
    }

    get nome():string {
        const msg = this.categoria.nome.completo
        const numSerie = this.numeroDeSerie?.completo ? `: ${this.numeroDeSerie.completo}` : ''
